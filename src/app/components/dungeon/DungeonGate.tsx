 ...
 * (see the login page's launch sequence). */
export default function DungeonGate({
  ...
  return (
    <section className="relative flex min-h-[calc(100vh-64px)] items-center justify-center overflow-hidden bg-[#0b0a08] text-white">
      {/* Stone vignette + fog */}
      <div aria-hidden className="pointer-events-none absolute inset-0">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_50%_30%,rgba(69,45,20,0.55),transparent_65%)]" />
        <div className="animate-fog-drift absolute inset-x-[-10%] top-1/2 h-[60%] bg-[radial-gradient(ellipse_at_center,rgba(120,110,90,0.14),transparent_70%)] blur-2xl" />
      ...
      <div aria-hidden className="pointer-events-none absolute left-[8%] top-1/3 hidden sm:block">
        <Flame className="animate-torch-flicker h-10 w-10 text-amber-500 drop-shadow-[0_0_18px_rgba(245,158,11,0.8)]" />
      </div>
      <div aria-hidden className="pointer-events-none absolute right-[8%] top-1/3 hidden sm:block">
        <Flame
          className="animate-torch-flicker h-10 w-10 text-amber-500 drop-shadow-[0_0_18px_rgba(245,158,11,0.8)]"
          style={{ animationDelay: '1.1s' }}
        />
      </div>

      <div className="relative mx-auto flex max-w-2xl flex-col items-center px-6 py-20 text-center">
        <div className="animate-fade-up mb-5 inline-flex items-center gap-2 rounded-full border border-amber-400/25 bg-amber-400/10 px-3 py-1">
        ...
        <button
          onClick={onEnter}
          className="animate-fade-up group mt-9 inline-flex items-center gap-2.5 rounded-full bg-amber-500 px-7 py-3.5 text-base font-semibold text-[#241a08] shadow-[0_0_30px_rgba(245,158,11,0.35)] transition-transform hover:scale-[1.03] hover:bg-amber-400 active:scale-95"
